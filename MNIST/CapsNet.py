...
class CapsLayer(layers.Layer):
    def __init__(self, caps_n, dims_n, rounds=3, **kwargs):
...
class CapsNet:
    ...
    def build(width, height, classes, routing_count):
        ...
        outputs = LengthLayer(name = 'outputs')(caps2)
...
model.compile(optimizer = 'adam', loss = [margin_loss, 'mse'], metrics = ['accuracy'])

file_in = open("input_data2")
...
test_pictures = np.reshape(test_pictures, (10000, 28, 28, 1))

file_in.close()
file_in2.close()

model.fit(x = train_pictures, y = train_labels, validation_data = (test_pictures, test_labels), shuffle = True, batch_size = 128, epochs = 4)

x, y = model.evaluate(x = test_pictures, y = test_labels)
